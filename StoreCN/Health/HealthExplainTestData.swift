...
class HealthExplainTestData {
     ...
     */
    private func _setFat(mDoubRs: Double) {
        // doubRs 格式為: 12.1
        var doubRs = mDoubRs
    ...
    func CalHealthData(strGroup: String, jobjItem: Dictionary<String, String>)->Dictionary<String, String> {
        ...
        if (strGroup == "bmi") {
            // 檢查 weigh, height
            if (mJobItem["weight"] == nil || mJobItem["height"] == nil) {
                return jobjItem
            }
        ...
        // 計算腰臀比
        if (strGroup == "whr") {
            if (mJobItem["waistline"] == nil || mJobItem["hipline"] == nil) {
                return jobjItem
            }
            
            let dbWhr = Double(Double(mJobItem["waistline"]!)! / Double(mJobItem["hipline"]!)!)
            mJobItem["whr"] = String(format: "%.2f", dbWhr)
